...
public class CucumberStepDefinitions {
    ...
    public void theFollowingAppointmentsExistInTheSystem(List<List<String>> list) {
        ...
        for(List<String> row :list){
            ...
            if(test){
                String[] serv = row.get(2).split(",");
                BookableService bookableService = BookableService.getWithName(row.get(1));
                if(bookableService instanceof ServiceCombo){
                    a.addChosenItem(((ServiceCombo)BookableService.getWithName(row.get(1))).getMainService());
                    for (ComboItem c:((ServiceCombo)BookableService.getWithName(row.get(1))).getServices()){
                        if(row.get(2).contains(c.getService().getName())){
                            a.addChosenItem(c);
                        }
                    }
                }
