		...
		InvalidInputException {
			...
			try {
				...
				if (user.equals(FlexiBookApplication.getUser())) {
					if (user.getUsername() == oldUsername && oldUsername != newUsername) {
	...
	**/
	...
	public static List<TOAppointmentCalendarItem> getAppointmentCalendarWeek(String date) throws ParseException, InvalidInputException {
		...
		public static List<TOAppointmentCalendarItem> getAppointmentCalendar (String date) throws
				...
				for (BusinessHour b : flexibook.getHours()) {
					if (b.getDayOfWeek().toString().equalsIgnoreCase(dayOfWeek.toString())) {
						...
						if (calendar.size() != 0) {
							...
							if (calendar.get(0).getDescription().equals("appointment")) {
								if (calendar.get(0).getStartTime().compareTo(b.getEndTime()) >= 0) {
						...
						counter = 0;
						while ((counter < calendar.size() - 1) && (calendar.get(counter).getEndTime().before(b.getEndTime()) || calendar.get(counter).getEndTime().equals(b.getEndTime()))) {
							if (!calendar.get(counter).getEndTime().equals(calendar.get(counter + 1).getStartTime())) {
								if (!calendar.get(counter).getDescription().equals("available") || !calendar.get(counter).getDescription().equals("business hours") ||
										!calendar.get(counter + 1).getDescription().equals("business hours") || !calendar.get(counter + 1).getDescription().equals("available")) {
									addItems.add(new TOAppointmentCalendarItem("business hours", sqlDate, calendar.get(counter).getEndTime(), calendar.get(counter + 1).getStartTime(), true, null, null));
								}
							}
							counter++;
						}
			...
			if(end != null){
				TOAppointmentCalendarItem endTime = new TOAppointmentCalendarItem("business hours", sqlDate,null,end,true,null,null);
				Time s = null;
			...
			while(!finish){
				for(int a = 0; a<calendar.size();a++){
					for(int b = 0;b<calendar.size();b++){
						if(a!= b){
							if(calendar.get(a).getUsername() != null && calendar.get(a).getUsername().equals(calendar.get(b).getUsername())){
								if(calendar.get(a).getStartTime().equals(calendar.get(b).getStartTime())){
									if(calendar.get(a).getDate().equals(calendar.get(b).getDate())){
										removeItems.add(calendar.get(a));
										finish = true;
										break;
									}
								}
			...
			calendar.removeAll(removeItems);
 ...
 */
					...
					if (BookableService.getWithName(serviceName) instanceof Service) {
						for (Appointment a : flexibook.getAppointments()) {
							...
							int dur = 0;
							BookableService se = a.getBookableService();
							if (se instanceof Service) {
								dur = ((Service) se).getDuration();
							} else {
					...
					//SERVICE COMBO
						...
						if (optionalServices != null || !optionalServices.equals("null")) {
		...
		public static void cancelAppointment (String customer, String apptType, String date, String time) throws InvalidInputException {
			...
			try {
				...
				Time sTime;
				if (time.length() == 7) {
					sTime = Time.valueOf(LocalTime.parse(time, DateTimeFormatter.ofPattern("k:mm:ss")));
				} else {
				...
				for(Appointment a:flexibook.getAppointments()){
					if(sDate.equals(a.getTimeSlot().getStartDate())){
						if(customer.equals(a.getCustomer().getUsername())){
							if(sTime.equals(a.getTimeSlot().getStartTime())){
								appt = a;
							}
						}
					}
				...
				if(appt != null){
					...
					if (appt.getTimeSlot().getStartDate().equals(current)) {
		 ...
		 * @throws InvalidInputException
		 * */
		public static void updateAppointment (String customer, String serviceType,String currentTime,String newTime, String newDate, String action,
											  String newComboItem) throws InvalidInputException {
			FlexiBook flexibook = FlexiBookApplication.getFlexiBook();
			try {
				Time cTime;
				if (currentTime.length() == 7) {
					cTime = Time.valueOf(LocalTime.parse(currentTime, DateTimeFormatter.ofPattern("k:mm:ss")));
				} else {
					cTime = Time.valueOf(LocalTime.parse(currentTime, DateTimeFormatter.ofPattern("kk:mm:ss")));
				}

				Appointment appt = null;
				for (Appointment a : flexibook.getAppointments()) {
					if (a.getCustomer().getUsername().equals(customer)) {
						if(a.getTimeSlot().getStartTime().equals(cTime)) {
							appt = a;
						}
				...
				if(Date.valueOf(SystemTime.getDate().toLocalDate()).equals(appt.getTimeSlot().getStartDate()) &&
					...
					throw new InvalidInputException("Cannot update an appointment on the appointment date");
				}
				if (customer.equals(flexibook.getOwner().getUsername())) {
				...
				//&& (serviceType == null || serviceType.equals("null")
